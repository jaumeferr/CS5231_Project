...
*   DESCRIPTION:
*       A first challenge for the CTF for the final project in the CS5231 course from NUS
*   COMPILATION:
*       gcc -m32 -ggdb -fno-stack-protector -mpreferred-stack-boundary=2 -mstack-protector-guard=global -L/usr/lib32 chall1.c -o chall1       
*   SETUP:
*       Please add a .canary file in the same folder with 4 bytes
*       You can perform the following commands so that you'll be in a more realistic environment
*        - sudo chown root:root .canary chall1
*        - sudo chmod u+s chall1
**********************************************************************************************************************************************/
...
#define LAUNCH_LENGTH 60
#define CANARY getCanary()
...
//-------------------------------------//
// Get the secret canary from the file //
//-------------------------------------//

int getCanary() {
	FILE* canary_file = fopen(".canary", "rb");
	int canary = 0;
	char c;
	while((c = fgetc(canary_file)) != EOF) {
		canary = (canary << 8) | (0xff & (int)c);
	}
	fclose(canary_file);
	return canary;
}
...
int main(int argc, char* argv[]) {
	...
	int canary = CANARY;						// We use a canary to prevent buffer overflow
	...
	if(canary != CANARY) {
		printf("The canary %p is died !!! Abort ! Abort !\n", canary);
		exit(1);
	}
